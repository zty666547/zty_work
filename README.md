...
| --- | ---: | ---: | ---: | ---: |
| 不追问 | 23.3% | 0.493 | 0.00 | 23.3% |
| 固定顺序 | 93.3% | 0.967 | 3.33 | 50.0% |
| 随机追问 | 92.0% | 0.960 | 3.50 | 36.3% |
| **信息增益** | **100.0%** | **1.000** | **3.10** | **63.3%** |

当前评测集包含三个故障族各10条、覆盖全部14个原因的30条来源约束合成路径，其中9条开发集、21条冻结测试集。测试集上信息增益策略Top-1为100%，固定顺序为90.5%，随机追问平均为90.4%。这说明主动追问明显优于直接猜测，并初步支持信息增益策略的效率优势；合成路径仍不能代表真实世界泛化能力。评测设计、指标解释与限制见[对比实验](docs/evaluation.md)。
