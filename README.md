...
基础因果图谱与证据层共有 **132 个节点、345 条关系**。最终演示使用的第二版服务感知图谱进一步加入服务、端点和部署环境，共有 **147 个节点、378 条关系、15 类节点和 27 类受控关系**。
...
第二版案例还会记录每个问题的“不清楚”比例和页面回答耗时，用于后续校准可回答率与检查成本。样本不足时只汇总、不改参数；风险继续单独人工审查。详见[问题效用参数校准](docs/question-calibration.md)。
...
第二版另增加一层独立服务上下文图谱。当前Open WebUI访问宿主机Ollama的案例已扩展到147个节点、378条关系，并能根据`Service`、`Endpoint`和`DeploymentContext`选择Ollama专属检查与修复。网页“固定案例”页提供节点类型直接标注、累计节点和稳定布局的三阶段截图模式。详见[服务感知图谱](docs/service-aware-graph.md)。

网页“图谱结构”页将15类节点分成故障与证据、交互推理、环境约束、方案输出四层，并从当前图谱实时列出27类关系。`OBSERVATION_SUPPORTS`与普通关联边被明确区分，用于说明条件概率如何驱动贝叶斯更新。详见[图谱结构与因果逻辑](docs/graph-explanation.md)。

网页“方法说明”页不再罗列模块，而是从固定轨迹实时提取初始候选、问题效用、条件概率更新、熵变化和停止阈值，用三个连续动作解释主动诊断算法。详见[算法三步解释](docs/algorithm-explanation.md)。

第二版的可回答性感知策略在42条已查看案例中改变了15条首问；在原28条开发案例中，Top-1由96.4%变为100%，但这些结果只用于解释算法行为，尚未经过新的外部测试集验证。详见[第二版信息增益分析](docs/v2-information-gain.md)。
