...
案例还新增一个诊断问题、一个观察、一个检查、一个修复、一个风险、三个官方来源节点和三个对应的证据片段。
...
| 阶段 | 系统行为 | 首位原因 | 概率 |
| --- | --- | --- | ---: |
| 初始 | 图文检索并识别服务上下文 | 服务地址配置错误 | 56.0% |
| 第一次回答 | 用户确认宿主机成功、容器失败 | 服务地址配置错误 | 95.2% |
| 第二次回答 | 用户确认必需变量并未缺失 | 服务地址配置错误 | 96.9% |
...
- 完成阶段：21个节点，只沿最终原因展开检查、修复、风险和Ollama/Open WebUI/Docker来源路径。

固定案例页采用累计子图：第二阶段保留第一阶段已使用的节点，第三阶段继续保留前两次问题和观察。每个节点直接标出“故障、原因、证据、问题、观察、服务、端点、部署环境、检查、修复、风险或来源”，三个阶段共用同一图骨架，便于生成位置稳定的截图。
