...
| `src/diagnosis/generator.py` | 声明—证据白名单、模型编排校验和确定性答案渲染 |
...
诊断状态保存`trajectory`阶段快照：初始融合概率、每轮实际问题与回答、观察节点、候选排名、熵、下一问和停止原因。快照与当前状态独立，刷新和回看不会追加记录或回滚诊断。无法确认的回答保留概率，但记录已询问的问题。

网页按阶段展示真实图谱边、概率曲线和排名变化；所有候选始终保留，灰色表示概率下降。结束阶段展开首位原因的检查、修复、风险和来源路径。达到轮数上限、信息不足或用户结束均不表示已确认根因。
...
每条已验证诊断计划被赋予`C1/C2/C3`声明ID，召回片段保留`E001…`证据ID。DeepSeek只允许输出声明的完整排列、已召回证据的子集以及`diagnosis/checks/safety`三种侧重点之一。程序会验证未知ID、重复证据、遗漏声明和非法侧重点，然后使用原始结构化计划渲染答案。模型不能直接输出原因、命令、修复动作或链接；任何不合法结果都会回退到离线模板。
